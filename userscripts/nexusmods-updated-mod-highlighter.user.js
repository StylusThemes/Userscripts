...
(function() {
  ...
  class NexusModsHighlighter {
    ...
    injectTableStyles() {
      ...
      injectStyles(css, 'nexus-updated-style');
    }
    ...
    injectTileStyles() {
      ...
      injectStyles(css, CONFIG.tile.styleId);
    }

    injectGlobalStyles() {
      const css = `*{border-radius:0!important}`;
      injectStyles(css, CONFIG.global.styleId);
    }
    ...
    processTable() {
      ...
      const tableRows = qsa('tr.even, tr.odd');
      let highlightedCount = 0;
      for (const tableRow of tableRows) {
        const downloadDateCell = qs('td.table-download', tableRow);
        const updateDateCell = qs('td.table-update', tableRow);
        if (!downloadDateCell || !updateDateCell) continue;
        const downloadTimestamp = parseDate(downloadDateCell.textContent);
        const updateTimestamp = parseDate(updateDateCell.textContent);
    ...
    setupMutationObserver() {
      ...
      this.mutationObserver = createMutationObserver(this.debouncedProcess);
    }
