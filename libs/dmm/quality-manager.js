// ==UserScript==
// @name          DMM Quality Manager Module
// @version       1.0.0
// @description   Quality selection module for DMM Trash Guide Regex Buttons
// @author        Journey Over
// @license       MIT
// ==/UserScript==
...
(function() {
  ...
  class QualityManager {
    ...
    async loadPersistedSettings() {
      try {
        const stored = await GMC.getValue(this.config.STORAGE_KEY, null);
        this.selectedOptions = stored ? JSON.parse(stored) : [];

        const polarityStored = await GMC.getValue(this.config.POLARITY_STORAGE_KEY, null);
        const polarityData = polarityStored ? JSON.parse(polarityStored) : {};
        this.qualityPolarity = new Map(Object.entries(polarityData));

        const logicStored = await GMC.getValue(this.config.LOGIC_STORAGE_KEY, null);
    ...
    onLogicChange(useAndLogic) {
      ...
      try {
        GMC.setValue(this.config.LOGIC_STORAGE_KEY, JSON.stringify(this.useAndLogic));
      } catch (err) {
    ...
    onToggleOption(key, btn) {
      ...
      try {
        GMC.setValue(this.config.STORAGE_KEY, JSON.stringify(this.selectedOptions));
        GMC.setValue(this.config.POLARITY_STORAGE_KEY, JSON.stringify(Object.fromEntries(this.qualityPolarity)));
      } catch (err) {
